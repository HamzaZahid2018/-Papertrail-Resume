...
const ResumeMiniPreview: React.FC<{ design: TemplateDesign }> = ({ design }) => {
  const SectionLabel = ({ children }: { children: React.ReactNode }) => (
    <span className="font-mono-plex text-[7px] uppercase tracking-[0.22em] font-bold" style={{ color: design.accent }}>
      {children}
    </span>
  );
  ...
  if (design.preview === "editorial") {
    return (
      <div className="h-56 rounded-xl border border-[#C9D3C6] bg-[#FBFAF5] p-4 shadow-inner overflow-hidden">
        <div className="border-b border-[#C9D3C6] pb-3 mb-3">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="font-fraunces text-lg leading-none text-[#1E3A34]">Nora Bennett</div>
              <div className="font-mono-plex text-[8px] uppercase tracking-[0.22em] mt-1" style={{ color: design.accent }}>Brand Strategist</div>
            </div>
            <div className="text-right font-mono-plex text-[7px] text-[#5B6B60] uppercase tracking-wider">
              <div>London</div>
              <div>nora.studio</div>
            </div>
          </div>
        </div>
        <div className="grid grid-cols-[1.3fr_0.8fr] gap-3 h-[calc(100%-3.5rem)]">
          <div className="space-y-3">
            <div>
              <SectionLabel>Profile</SectionLabel>
              <p className="mt-1 text-[8px] leading-relaxed text-[#3E5750]">Editorial thinker shaping positioning, launches, and multi-channel campaigns.</p>
            </div>
            <div>
              <SectionLabel>Experience</SectionLabel>
              <div className="mt-1.5 space-y-2">
                <div>
                  <div className="flex justify-between text-[8px] font-semibold text-[#1E3A34]">
                    <span>Studio North</span>
                    <span className="text-[#5B6B60] font-normal">2023</span>
                  </div>
                  <div className="text-[7px] text-[#5B6B60]">Campaign systems and launch playbooks</div>
                </div>
                ...
                <div>
                  ...
                  <div className="text-[7px] text-[#5B6B60]">Narrative direction for luxury brands</div>
                </div>
              </div>
            </div>
          </div>
          <div className="bg-[#EFF3EC] rounded-lg p-2.5 space-y-2 border border-[#C9D3C6]">
            <div>
              <SectionLabel>Skills</SectionLabel>
              <div className="mt-1 flex flex-wrap gap-1">
                {["Messaging", "Launch", "Copy"].map((item) => (
                  <span key={item} className="px-1.5 py-0.5 rounded bg-white text-[6px] font-semibold text-[#1E3A34] border border-[#C9D3C6]">
                    {item}
                  </span>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="mt-1 text-[7px] text-[#5B6B60] leading-relaxed">D&AD shortlist<br />Webby Honoree</div>
            </div>
          </div>
        </div>
      </div>
  ...
  if (design.preview === "minimal") {
    return (
      <div className="h-56 rounded-xl border border-[#C9D3C6] bg-[#FBFAF5] p-4 overflow-hidden">
        <div className="flex items-center justify-between pb-3 border-b border-[#C9D3C6]">
          <div>
            <div className="font-fraunces text-lg leading-none text-[#1E3A34]">Avery Khan</div>
            <div className="font-mono-plex text-[8px] uppercase tracking-[0.24em] mt-1 text-[#5B6B60]">Product Analyst</div>
          </div>
          <div className="w-8 h-8 rounded-full border-2" style={{ borderColor: design.accent }} />
        </div>
        <div className="pt-3 space-y-3">
          <div>
            <SectionLabel>Summary</SectionLabel>
            <p className="mt-1 text-[8px] leading-relaxed text-[#3E5750]">Metrics-focused operator translating messy data into simple hiring stories.</p>
          </div>
          <div>
            ...
            <div className="mt-1.5 space-y-2">
              {["Activation reporting", "Funnel optimization", "Forecast modeling"].map((item, index) => (
                <div key={item} className="flex items-center gap-2">
                  <span className="w-3 h-3 rounded-full text-[6px] flex items-center justify-center font-bold text-[#1E3A34]" style={{ backgroundColor: index === 1 ? "#F1D9A5" : "#EFF3EC" }}>
                    {index + 1}
                  </span>
                  <span className="text-[7px] text-[#5B6B60]">{item}</span>
                </div>
              ))}
            </div>
          </div>
          <div className="grid grid-cols-3 gap-2 pt-1">
            {["SQL", "Looker", "A/B Tests"].map((item) => (
              <div key={item} className="rounded-md border border-[#C9D3C6] bg-[#EFF3EC] px-1.5 py-1 text-center text-[6px] font-semibold text-[#1E3A34]">{item}</div>
            ))}
          </div>
        </div>
      </div>
  ...
  if (design.preview === "sidebar") {
    return (
      <div className="h-56 rounded-xl border border-[#C9D3C6] bg-[#FBFAF5] overflow-hidden grid grid-cols-[0.78fr_1.22fr]">
        <div className="bg-[#1E3A34] text-[#FBFAF5] p-3 space-y-3">
          <div>
            <div className="font-fraunces text-base leading-none">Mila Torres</div>
            <div className="font-mono-plex text-[7px] uppercase tracking-[0.24em] mt-1 text-[#F1D9A5]">Finance Lead</div>
          </div>
          <div>
            <div className="font-mono-plex text-[7px] uppercase tracking-[0.2em] text-[#F1D9A5]">Contact</div>
            <div className="mt-1 text-[7px] leading-relaxed text-[#E4EAE0]">mila@ledger.co<br />New York</div>
          </div>
          <div>
            <div className="font-mono-plex text-[7px] uppercase tracking-[0.2em] text-[#F1D9A5]">Tools</div>
            <div className="mt-1 flex flex-wrap gap-1">
              {["FP&A", "SQL", "ERP"].map((item) => (
                <span key={item} className="bg-[#2C5F5B] px-1.5 py-0.5 rounded text-[6px] font-semibold">{item}</span>
              ))}
            </div>
          </div>
        </div>
        <div className="p-4 space-y-3">
          <div>
            <SectionLabel>Profile</SectionLabel>
            <p className="mt-1 text-[8px] leading-relaxed text-[#3E5750]">Budgeting and reporting partner for scaling teams across operations and planning.</p>
          </div>
          <div>
            <SectionLabel>Experience</SectionLabel>
            <div className="mt-1.5 space-y-2">
              <div>
                <div className="text-[8px] font-semibold text-[#1E3A34]">Northline Capital</div>
                <div className="text-[7px] text-[#5B6B60]">Built quarterly planning packs and KPI reviews</div>
              </div>
              <div>
                <div className="text-[8px] font-semibold text-[#1E3A34]">Atlas Energy</div>
                <div className="text-[7px] text-[#5B6B60]">Owned expense controls and board-ready summaries</div>
              </div>
            </div>
          </div>
          <div className="h-[1px] bg-[#C9D3C6]" />
          <div className="grid grid-cols-2 gap-2 text-[7px] text-[#5B6B60]">
  ...
  if (design.preview === "timeline") {
    return (
      <div className="h-56 rounded-xl border border-[#C9D3C6] bg-[#FBFAF5] p-4 overflow-hidden">
        <div className="pb-3 border-b border-[#C9D3C6]">
          <div className="font-fraunces text-lg leading-none text-[#1E3A34]">Jonas Reed</div>
          <div className="font-mono-plex text-[8px] uppercase tracking-[0.24em] mt-1" style={{ color: design.accent }}>Engineering Director</div>
        </div>
        <div className="pt-3 space-y-3">
          <SectionLabel>Career Timeline</SectionLabel>
          <div className="relative ml-2 border-l border-[#C9D3C6] pl-4 space-y-3">
            {[
              ...
              <div key={year} className="relative">
                <span className="absolute -left-[1.15rem] top-1 w-2.5 h-2.5 rounded-full border-2 bg-[#FBFAF5]" style={{ borderColor: index === 0 ? design.accent : "#C9D3C6" }} />
                <div className="text-[7px] font-mono-plex font-bold uppercase tracking-wider" style={{ color: design.accent }}>{year}</div>
                <div className="text-[8px] text-[#3E5750] leading-relaxed">{text}</div>
              </div>
            ))}
          </div>
        </div>
  ...
  if (design.preview === "portfolio") {
    return (
      <div className="h-56 rounded-xl border border-[#C9D3C6] bg-[#FBFAF5] p-4 overflow-hidden">
        <div className="flex justify-between items-start gap-4 pb-3 border-b border-[#C9D3C6]">
          <div>
            <div className="font-fraunces text-lg leading-none text-[#1E3A34]">Sana Malik</div>
            <div className="font-mono-plex text-[8px] uppercase tracking-[0.24em] mt-1 text-[#5B6B60]">Product Designer</div>
          </div>
          <div className="px-2 py-1 rounded-full text-[6px] font-mono-plex font-bold uppercase tracking-wider text-white" style={{ backgroundColor: design.accent }}>
            Featured work
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2 pt-3">
          {[
            ...
            <div key={title} className="rounded-lg border border-[#C9D3C6] p-2" style={{ backgroundColor: bg }}>
              <div className="h-10 rounded-md bg-white/80 border border-[#C9D3C6]" />
              <div className="mt-2 text-[7px] font-semibold text-[#1E3A34]">{title}</div>
              <div className="text-[6px] text-[#5B6B60]">Prototype, metrics, rollout</div>
            </div>
          ))}
        </div>
      </div>
  ...
  if (design.preview === "compact") {
    return (
      <div className="h-56 rounded-xl border border-[#C9D3C6] bg-[#FBFAF5] p-4 overflow-hidden">
        <div className="flex items-center justify-between pb-2 border-b-2" style={{ borderColor: design.accent }}>
          <div>
            <div className="font-fraunces text-lg leading-none text-[#1E3A34]">Elena Frost</div>
            <div className="font-mono-plex text-[8px] uppercase tracking-[0.22em] mt-1 text-[#5B6B60]">Chief of Staff</div>
          </div>
          <div className="text-[7px] font-mono-plex uppercase tracking-widest text-[#5B6B60]">1 page</div>
        </div>
        <div className="grid grid-cols-[1.2fr_0.8fr] gap-3 pt-3">
          <div className="space-y-3">
            <div>
              <SectionLabel>Highlights</SectionLabel>
              <ul className="mt-1 space-y-1 text-[7px] text-[#3E5750]">
                <li>Led annual planning cadence across four functions</li>
                <li>Built board memo templates and decision trackers</li>
                <li>Reduced hiring ops turnaround by 31%</li>
              </ul>
            </div>
            <div>
              <SectionLabel>Experience</SectionLabel>
              <div className="mt-1 text-[8px] font-semibold text-[#1E3A34]">Orbit Health</div>
              <div className="text-[7px] text-[#5B6B60]">Chief of Staff to COO</div>
            </div>
          </div>
          <div className="space-y-2">
            <div className="rounded-lg border border-[#C9D3C6] bg-[#EFF3EC] p-2">
              <div className="text-[6px] font-mono-plex uppercase tracking-wider text-[#5B6B60]">Core</div>
              <div className="mt-1 text-[7px] font-semibold text-[#1E3A34]">Planning, Comms, Recruiting</div>
            </div>
            <div className="rounded-lg border border-[#C9D3C6] bg-[#EFF3EC] p-2">
              <div className="text-[6px] font-mono-plex uppercase tracking-wider text-[#5B6B60]">Education</div>
              <div className="mt-1 text-[7px] font-semibold text-[#1E3A34]">Georgetown</div>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="h-56 rounded-xl border border-[#C9D3C6] bg-[#FBFAF5] p-4 shadow-inner overflow-hidden">
      <div className="border-b border-[#C9D3C6] pb-3">
        <div className="font-fraunces text-lg leading-none text-[#1E3A34]">Ethan Cole</div>
        <div className="font-mono-plex text-[8px] uppercase tracking-[0.24em] mt-1" style={{ color: design.accent }}>Senior Product Manager</div>
        <div className="mt-1 text-[7px] text-[#5B6B60] font-mono-plex">ethan.pm | Seattle | linkedin.com/in/ethan</div>
      </div>
      <div className="pt-3 space-y-3">
        <div>
          <SectionLabel>Summary</SectionLabel>
          <p className="mt-1 text-[8px] leading-relaxed text-[#3E5750]">Product operator focused on hiring workflows, growth systems, and clear communication.</p>
        </div>
        <div>
          <SectionLabel>Experience</SectionLabel>
          <div className="mt-1.5 space-y-2">
            <div>
              <div className="flex justify-between text-[8px] font-semibold text-[#1E3A34]">
                ...
                <span className="text-[#5B6B60] font-normal">2022-Present</span>
              </div>
              <div className="text-[7px] text-[#5B6B60]">Owned roadmap for recruiter workflow automation</div>
            </div>
            <div>
              <div className="flex justify-between text-[8px] font-semibold text-[#1E3A34]">
                <span>Mode Studio</span>
                <span className="text-[#5B6B60] font-normal">2019-2022</span>
              </div>
              <div className="text-[7px] text-[#5B6B60]">Launched self-serve dashboards for hiring teams</div>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap gap-1 pt-1">
          {["Roadmaps", "Research", "SQL", "Hiring"].map((item) => (
            <span key={item} className="rounded-full border border-[#C9D3C6] bg-[#EFF3EC] px-1.5 py-0.5 text-[6px] font-semibold text-[#1E3A34]">{item}</span>
          ))}
        </div>
      </div>
    </div>
              ...
              >
                <div className="bg-[#FBFAF5] border border-[#C9D3C6] rounded-[1.75rem] p-5 hover:-translate-y-2 hover:shadow-lg transition-all duration-300 group cursor-pointer h-full">
                  ...
                  <div className="flex justify-between items-start gap-3 mt-5">
                    ...
                    <span className="font-mono-plex text-xs text-[#5B6B60] pt-1">{design.number}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="bg-[#FBFAF5] p-8 rounded-3xl border border-[#C9D3C6] shadow-sm">
        ...
        <div className="flex bg-[#E4EAE0] rounded-xl p-1 mb-6">
          <button
            ...
            className={`flex-1 py-2.5 rounded-lg text-xs font-bold font-mono-plex uppercase tracking-wider transition-all duration-200 ${
              mode === "signin"
